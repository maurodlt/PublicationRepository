...
from sqlalchemy import (
    ...
    Time, DateTime, Float, Integer, Enum, text
)
...
class Conference(Publication):
    ...
    id: Mapped[int] = mapped_column(ForeignKey("publication.id", ondelete="CASCADE"), primary_key=True)
...
# Database connection
DB_PATH = Path(__file__).resolve().parent / "data" / "Class_Diagram.db"
DB_PATH.parent.mkdir(parents=True, exist_ok=True)
DATABASE_URL = f"sqlite:///{DB_PATH}"  # SQLite connection
engine = create_engine(DATABASE_URL, echo=True)

# Create tables in the database
Base.metadata.create_all(engine, checkfirst=True)

# Ensure the citations column exists in the publication table for older databases
