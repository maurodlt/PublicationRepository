...
from fastapi import Depends, FastAPI, HTTPException, Request, status, Body, UploadFile, File
...
from bibtex_handler import process_bibtex_import
...
def init_db():
    ...
    if database_path.startswith("sqlite:///./"):
        db_file = Path(database_path.replace("sqlite:///./", ""))
    ...
    if engine.url.get_backend_name() == "sqlite":
        with engine.begin() as conn:
            result = conn.execute(text("PRAGMA table_info(publication)"))
            columns = [row[1] for row in result.fetchall()]
            if "citations" not in columns:
                conn.execute(text("ALTER TABLE publication ADD COLUMN citations INTEGER"))
...
@app.post("/import/bibtex/", tags=["System"])
async def import_bibtex(file: UploadFile = File(...), database: Session = Depends(get_db)):
    """Upload a .bib file and import its entries into the database."""
    ...
    logger.info(f"Starting BibTeX import for file {file.filename}")
    result = await process_bibtex_import(text, database)
    logger.info(f"BibTeX import completed: {result['created']}")
    return result
...
@app.get("/institution-author-counts/", response_model=None, tags=["Institution"])
def get_institution_author_counts(database: Session = Depends(get_db)) -> list:
    """Get institutions sorted by author count descending"""
    author_counts = (
        database.query(
            Institution.id.label("institution_id"),
            Institution.name,
            Institution.country,
            func.count(author_institution.c.author).label("author_count")
        )
        .outerjoin(author_institution, Institution.id == author_institution.c.institution)
        .group_by(Institution.id)
        .order_by(func.count(author_institution.c.author).desc())
        .all()
    )
...
@app.get("/institution-publication-counts/", response_model=None, tags=["Institution"])
def get_institution_publication_counts(database: Session = Depends(get_db)) -> list:
    """Get institutions sorted by publication count descending"""
    publication_counts = (
        database.query(
            Institution.id.label("institution_id"),
            Institution.name,
            Institution.country,
            func.count(publication_institution.c.publication).label("publication_count")
        )
        .outerjoin(publication_institution, Institution.id == publication_institution.c.institution_1)
        .group_by(Institution.id)
        .order_by(func.count(publication_institution.c.publication).desc())
        .all()
    )
...
async def create_institution(institution_data: InstitutionCreate, database: Session = Depends(get_db)) -> Institution:
    ...
    existing_institution = database.query(Institution).filter(
    ...
    db_institution = Institution(
        country=institution_data.country,        city=institution_data.city,        name=institution_data.name        )
...
async def bulk_create_institution(items: list[InstitutionCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_institution = Institution(
                country=item_data.country,                city=item_data.city,                name=item_data.name            )
            database.add(db_institution)
...
@app.get("/publication/citations/", response_model=None, tags=["Publication"])
def get_publication_citations(database: Session = Depends(get_db)) -> list:
    """Get publications with citations > 0, sorted by citations descending"""
    publications = (
        database.query(Publication)
        .filter(Publication.citations > 0)
...
@app.get("/author/citations/", response_model=None, tags=["Author"])
def get_author_citations(database: Session = Depends(get_db)) -> list:
    """Get authors with their total citations from all publications, sorted by total citations descending"""
    author_citations = (
        database.query(
            Author.id.label("author_id"),
            Author.name,
            Author.last_name,
            func.coalesce(func.sum(Publication.citations), 0).label("total_citations")
        )
        .outerjoin(author_publication, Author.id == author_publication.c.author_1)
        .outerjoin(Publication, author_publication.c.publication_1 == Publication.id)
        .group_by(Author.id)
        .having(func.coalesce(func.sum(Publication.citations), 0) > 0)
        .order_by(func.coalesce(func.sum(Publication.citations), 0).desc())
        .all()
    )
    ...
    return [
        {
            ...
            "full_name": f"{row.last_name}, {row.name}",
            "name": row.name,
            "last_name": row.last_name,
            "total_citations": row.total_citations,
        }
        for row in author_citations
    ]
...
@app.get("/publication/total-citations/", response_model=None, tags=["Publication"])
def get_total_citations(database: Session = Depends(get_db)) -> list:
    """Get the total sum of all citations from all publications"""
    total_citations = database.query(func.coalesce(func.sum(Publication.citations), 0)).scalar()
...
@app.post("/publication/update-citations/", tags=["Publication"])
def run_publication_citation_update(
    payload: CitationUpdateRequest,
    database: Session = Depends(get_db)
):

    try:
        result = update_publication_citations(database, payload.email)
        return result

    except Exception as exc:
        logger.exception("Citation update failed")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update citations: {exc}"
        )

#@app.post("/publication/update-citations/", response_model=None, tags=["Publication"])
#def run_publication_citation_update(database: Session = Depends(get_db)) -> dict:
#    """Fetch citation counts from OpenAlex via pyalex and update all publications."""
#    try:
#        result = update_publication_citations(database)
#    except Exception as exc:
#        raise HTTPException(status_code=500, detail=f"Failed to update citations: {exc}")
...
@app.get("/author/publication-counts/", response_model=None, tags=["Author"])
def get_author_publication_counts(database: Session = Depends(get_db)) -> list:
    """Get authors sorted by publication count descending"""
    publication_counts = (
        database.query(
            Author.id.label("author_id"),
            Author.name,
            Author.last_name,
            func.count(author_publication.c.publication_1).label("publication_count")
        )
        .outerjoin(author_publication, Author.id == author_publication.c.author_1)
        .group_by(Author.id)
        .order_by(func.count(author_publication.c.publication_1).desc())
        .all()
    )
...
async def create_author(author_data: AuthorCreate, database: Session = Depends(get_db)) -> Author:
    # Check for duplicate author by name and last_name
    existing_author = database.query(Author).filter(
    ...
    if existing_author:
        raise HTTPException(
            status_code=409,
            detail=f"Author with name '{author_data.name} {author_data.last_name}' already exists"
        )
...
async def create_publication(publication_data: PublicationCreate, database: Session = Depends(get_db)) -> Publication:
    ...
    db_publication = Publication(
        ...
        citations=publication_data.citations
    )
...
async def update_publication(publication_id: int, publication_data: PublicationCreate, database: Session = Depends(get_db)) -> Publication:
    ...
    setattr(db_publication, 'citations', publication_data.citations)
...
async def create_conference(conference_data: ConferenceCreate, database: Session = Depends(get_db)) -> Conference:
    ...
    db_conference = Conference(
        ...
        citations=conference_data.citations,
        organization=conference_data.organization,
        publisher=conference_data.publisher,
        pages=conference_data.pages,
        booktitle=conference_data.booktitle,
        series=conference_data.series,
        editor=conference_data.editor,
        month=conference_data.month,
        address=conference_data.address,
        note=conference_data.note,
        number=conference_data.number
    )
...
async def bulk_create_conference(items: list[ConferenceCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_conference = Conference(
                title=item_data.title,                year=item_data.year,                organization=item_data.organization,                publisher=item_data.publisher,                pages=item_data.pages,                booktitle=item_data.booktitle,                series=item_data.series,                editor=item_data.editor,                month=item_data.month,                address=item_data.address,                note=item_data.note,                number=item_data.number            )
            database.add(db_conference)
            database.flush()  # Get ID without committing
...
async def create_journal(journal_data: JournalCreate, database: Session = Depends(get_db)) -> Journal:
    ...
    db_journal = Journal(
        ...
        citations=journal_data.citations,
...
async def create_proceedings(proceedings_data: ProceedingsCreate, database: Session = Depends(get_db)) -> Proceedings:
    ...
    db_proceedings = Proceedings(
        title=proceedings_data.title,
        year=proceedings_data.year,
        citations=proceedings_data.citations,
        booktitle=proceedings_data.booktitle,
        editor=proceedings_data.editor,
        address=proceedings_data.address,
        organization=proceedings_data.organization,
        number=proceedings_data.number,
        volume=proceedings_data.volume,
        pages=proceedings_data.pages,
        series=proceedings_data.series,
        publisher=proceedings_data.publisher,
        month=proceedings_data.month
    )
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        ...
        year=book_data.year,
        citations=book_data.citations,
        address=book_data.address,
...
async def create_thesis(thesis_data: ThesisCreate, database: Session = Depends(get_db)) -> Thesis:
    ...
    db_thesis = Thesis(
        ...
        citations=thesis_data.citations,
        address=thesis_data.address,
        month=thesis_data.month,
        note=thesis_data.note,
...
async def create_others(others_data: OthersCreate, database: Session = Depends(get_db)) -> Others:
    ...
    db_others = Others(
        title=others_data.title,
        year=others_data.year,
        citations=others_data.citations,
        peer_reviewed=others_data.peer_reviewed,
        link=others_data.link,
        server=others_data.server
    )
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_others = Others(
                ...
                year=item_data.year,
                citations=item_data.citations,
                peer_reviewed=item_data.peer_reviewed,
                link=item_data.link,
